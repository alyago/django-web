	...
	function makeupBalloon($target, $balloon, options) {
		...
		if(options.tipSize > 0) {
			...
			if(m) {
				...
				makeTip(balloon, outerTip, m, options.tipSize, $balloon.css("border-" + m.pos.o + "-color"), false);
				makeTip(balloon, innerTip, m, options.tipSize - balloon["border" + m.camel.pos.o], $balloon.css("background-color"), true);
		...
		// Make up balloon tip.
		function makeTip(balloon, tip, m, tipSize, color, isInner) {
			var len = Math.round(tipSize / 1.7320508);
			tip.inactive()
				["setBorder" + m.camel.pos.f](tipSize)
				["setBorder" + m.camel.pos.c1](len)
				["setBorder" + m.camel.pos.c2](len)
				["set" + m.camel.pos.p1]((m.isTopLeft) ? (isInner ? -tipSize + 1 : -tipSize) : (isInner ? balloon.inner[m.size.p] - 1 : balloon.inner[m.size.p]))
