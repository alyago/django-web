...
# Write the tests
class BugFix113Test(Test):
    def test_feature(self, query, bridge_response, app_response, soup):
        # Search Title should be myresume
        search_title_tag = soup.find('h1', 'search_title')
        if search_title_tag.contents[0].strip().startswith('Personalized search results'):
            pass
        else:
            self.messages.append('Bug 113 not fixed')
